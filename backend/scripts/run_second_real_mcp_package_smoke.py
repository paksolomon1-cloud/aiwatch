...
SESSION_ID = "stdio-real-package-memory-001"
DEFAULT_PACKAGE = "@modelcontextprotocol/server-memory@2026.1.26"
EXPECTED_TOOL_NAMES = {
    "create_entities",
    "create_relations",
    "add_observations",
    ...
    return [
        {
            ...
            "params": {
                ...
                "capabilities": {},
                "clientInfo": {"name": "aiwatch-second-real-package-smoke", "version": "0.1.0"},
            },
        },
        {
            "jsonrpc": "2.0",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run a second no-token real MCP package through the AIWatch stdio wrapper."
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        process.kill()
        sys.stderr.write("Second real MCP package smoke timed out; killed wrapper process.\n")
        return 1
    ...
    if process.returncode != 0:
        sys.stderr.write(f"Second real MCP package smoke failed with exit code {process.returncode}.\n")
    ...
    try:
        ...
        sys.stderr.write(f"Second real MCP package smoke could not verify backend state: {error}\n")
        return 1

    observed_tools = _objects_for_server(tools, SERVER_ID)
    observed_tool_names = {str(tool.get("tool_name", "")) for tool in observed_tools}
    missing_tools = sorted(EXPECTED_TOOL_NAMES - observed_tool_names)
    if missing_tools:
        sys.stderr.write(f"Missing expected tools for server_id={SERVER_ID}: {', '.join(missing_tools)}\n")
        return 1
    ...
    tool_names = ", ".join(sorted(observed_tool_names))
    response_count = len([line for line in stdout_data.splitlines() if line.strip()])
    print(f"Second real MCP package smoke completed with {response_count} protocol responses.")
